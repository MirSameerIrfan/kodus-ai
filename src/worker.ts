...
async function bootstrap() {
    ...
    const app = await NestFactory.createApplicationContext(WorkerModule, {
        logger: false, // Usaremos PinoLoggerService customizado
    });
    ...
    // Verifica se worker está habilitado
    ...
    pinoLogger.log({
        ...
        context: 'WorkerBootstrap',
        metadata: {
            environment: environment.API_CLOUD_MODE ? 'CLOUD' : 'SELF-HOSTED',
    ...
    process.on('uncaughtException', (error) => {
        ...
        // Não encerra o processo imediatamente - deixa PM2 gerenciar
    });
    ...
    // Worker fica rodando - RabbitMQ consumers e OutboxRelayService são iniciados automaticamente
    // via decorators e lifecycle hooks do NestJS
    pinoLogger.log({
